...
const PHASES = [
  {
    ...
    checklist: [
      ['gain-cp', 'Both players gain 1 Command Point.'],
...
let ui = {
  ...
  rulesQuery: '',
  rulesCategory: 'All',
  woundResult: 'Enter Strength and Toughness.',
...
function loadState() {
  try {
    ...
    console.warn('Could not load saved WAR Companion state.', error);
    return defaultState();
  }
}
...
function saveState() {
  try {
    ...
    console.error('Could not save WAR Companion state.', error);
...
function getTotals(game = state.game) {
  const totals = [0, 0];
  if (!game) return totals;
  for (const event of game.scoreEvents || []) {
    if (event.player === 0 || event.player === 1) totals[event.player] += Number(event.amount) || 0;
  }
  return totals;
}
...
function phaseKey(game = state.game) {
  return `${game.round}-${game.activePlayer}-${game.phaseIndex}`;
}

function commandGrantKey(game = state.game) {
  return `${game.round}-${game.activePlayer}`;
}
...
function grantCommandCP(game, announce = true) {
  ...
  game.players.forEach((player, index) => {
    player.cp = Math.max(0, Number(player.cp || 0) + 1);
    game.cpEvents.push({ id: uid('cp'), player: index, amount: 1, round: game.round, phase: 'Command', reason: 'Start of Command phase', ts: Date.now() });
  });
  game.cpGrants.push(key);
  game.phaseChecks ||= {};
  game.phaseChecks[phaseKey(game)] ||= {};
  game.phaseChecks[phaseKey(game)]['gain-cp'] = true;
  if (announce) setTimeout(() => showToast('Command phase: both players gained 1 CP.'), 30);
  return true;
}

function createGame(form) {
  const data = new FormData(form);
  const playerOne = String(data.get('playerOne') || 'Player 1').trim() || 'Player 1';
  const playerTwo = String(data.get('playerTwo') || 'Player 2').trim() || 'Player 2';
  const firstPlayer = Number(data.get('firstPlayer') || 0) === 1 ? 1 : 0;
  ...
  state.game = {
    ...
    phaseIndex: 0,
    players: [
      { name: playerOne, army: String(data.get('armyOne') || '').trim(), cp: startingCP },
      { name: playerTwo, army: String(data.get('armyTwo') || '').trim(), cp: startingCP }
    ],
...
function advancePhase() {
  ...
  mutateGame(g => {
    if (g.phaseIndex < PHASES.length - 1) {
      g.phaseIndex += 1;
    } else if (g.activePlayer === g.firstPlayer) {
      g.activePlayer = 1 - g.firstPlayer;
      g.phaseIndex = 0;
    } else {
      g.round += 1;
    ...
    grantCommandCP(g, true);
  });
}

function isFinalTurnPhase(game = state.game) {
  return game && game.round >= game.maxRounds && game.phaseIndex === PHASES.length - 1 && game.activePlayer !== game.firstPlayer;
}
...
function finishGame() {
  ...
  const archive = {
    ...
    players: game.players.map(player => ({ name: player.name, army: player.army })),
    totals,
    rounds: game.round,
...
function addScore(player, amount, category = 'Adjustment', note = '') {
  const numericAmount = Number(amount);
  if (!state.game || !Number.isFinite(numericAmount) || numericAmount === 0) return;
  mutateGame(game => {
    game.scoreEvents.push({
      id: uid('score'), player: Number(player), amount: numericAmount,
      category: category || 'Adjustment', note: note || '', round: game.round,
      phase: PHASES[game.phaseIndex].title, ts: Date.now()
    });
  });
}

function adjustCP(playerIndex, amount, reason = 'Manual adjustment') {
  if (!state.game) return;
  const index = Number(playerIndex);
  const current = Number(state.game.players[index].cp || 0);
  const next = current + Number(amount);
  if (next < 0) {
    showToast(`${state.game.players[index].name} does not have enough CP.`);
    return;
  }
  mutateGame(game => {
    game.players[index].cp = next;
    game.cpEvents.push({
      id: uid('cp'), player: index, amount: Number(amount), round: game.round,
      phase: PHASES[game.phaseIndex].title, reason, ts: Date.now()
    });
  });
}

function cycleObjective(index) {
  const order = ['neutral', 'p0', 'p1', 'contested'];
  mutateGame(game => {
...
function matchingReminders(game = state.game) {
  ...
  return state.reminders.filter(reminder => {
    ...
    if (reminder.scope === 'mine' && game.activePlayer !== 0) return false;
    if (reminder.scope === 'opponent' && game.activePlayer !== 1) return false;
    return true;
  });
}

function winnerText(game = state.game) {
  const totals = getTotals(game);
  if (totals[0] > totals[1]) return `${game.players[0].name} wins`;
  if (totals[1] > totals[0]) return `${game.players[1].name} wins`;
...
function renderSetup() {
  return `
    <main class="hero-setup">
      <img class="setup-brand" src="assets/brand-logo.webp" alt="WAR Companion">
      <section class="setup-panel">
        <div class="setup-panel-head">
          <h1>Begin a New Battle</h1>
          <p>Track the game without replacing your army roster or datasheets.</p>
        </div>
        <form id="new-game-form" class="setup-form stack">
          <div class="grid-2">
            <div class="field"><label for="playerOne">Your name</label><input id="playerOne" name="playerOne" value="Player 1" maxlength="40" required></div>
            <div class="field"><label for="playerTwo">Opponent</label><input id="playerTwo" name="playerTwo" value="Player 2" maxlength="40" required></div>
          </div>
          <div class="grid-2">
            <div class="field"><label for="armyOne">Your army <span class="help">(optional)</span></label><input id="armyOne" name="armyOne" placeholder="e.g., Space Marines" maxlength="60"></div>
            <div class="field"><label for="armyTwo">Opponent army <span class="help">(optional)</span></label><input id="armyTwo" name="armyTwo" placeholder="e.g., Orks" maxlength="60"></div>
          </div>
          ...
          <div class="grid-3">
            <div class="field"><label for="firstPlayer">First turn</label><select id="firstPlayer" name="firstPlayer"><option value="0">Player 1</option><option value="1">Player 2</option></select></div>
            <div class="field"><label for="startingCP">Starting CP</label><input id="startingCP" name="startingCP" type="number" min="0" max="99" value="0" inputmode="numeric"></div>
          ...
          <p class="help">The active battle is saved locally on this device and remains available offline after installation.</p>
        </form>
      </section>
    </main>`;
}
  ...
  return `
    <header class="topbar">
      <div class="topbar-brand">
        <img class="topbar-logo" src="assets/brand-logo.webp" alt="WAR Companion">
        <div class="topbar-title">
          <strong>${esc(game.mission)}</strong>
          <span>${esc(game.players[0].name)} vs ${esc(game.players[1].name)}</span>
        </div>
      </div>
...
function renderBattle() {
  ...
  const nextLabel = isFinalTurnPhase(game)
    ...
    : game.phaseIndex < PHASES.length - 1
      ...
      : game.activePlayer === game.firstPlayer
        ? `End Turn: ${game.players[1 - game.firstPlayer].name}`
  ...
  return `
    <section class="battle-status">
      ...
      <button class="turn-chip" type="button" data-action="switch-turn" title="Tap to correct the active player">
        Active Turn<strong>${esc(game.players[game.activePlayer].name)}</strong>
      </button>
    ...
    <section class="player-grid">
      ${game.players.map((player, index) => renderPlayerCard(player, index, totals[index], game.activePlayer === index)).join('')}
    </section>
    ...
    <section class="panel">
      <h2 class="section-title"><span>Quick Reference</span><img class="section-icon" src="assets/icons/search.png" alt=""></h2>
      <div class="btn-row">
        <button class="btn secondary" type="button" data-action="open-rules" data-query="wound roll">Wound Chart</button>
...
function renderPlayerCard(player, index, total, active) {
  return `
    <article class="player-card ${active ? 'active' : ''}">
      <div class="player-head"><strong>${esc(player.name)}</strong><span>${esc(player.army || (index === 0 ? 'Your army' : 'Opponent army'))}</span></div>
      <div class="resource-grid">
        <div class="resource">
          <div class="resource-label"><img src="assets/icons/vp.png" alt="">VP</div>
          <div class="resource-value">${total}</div>
          <div class="counter-row">
            <button class="counter-btn minus" type="button" data-action="quick-score" data-player="${index}" data-amount="-1">−1</button>
            <button class="counter-btn plus" type="button" data-action="quick-score" data-player="${index}" data-amount="1">+1</button>
            <button class="counter-btn plus" type="button" data-action="quick-score" data-player="${index}" data-amount="5">+5</button>
          </div>
        </div>
        <div class="resource cp">
          <div class="resource-label"><img src="assets/icons/cp.png" alt="">CP</div>
          <div class="resource-value">${Number(player.cp || 0)}</div>
          <div class="counter-row">
            <button class="counter-btn minus" type="button" data-action="quick-cp" data-player="${index}" data-amount="-1">−1</button>
            <button class="counter-btn plus" type="button" data-action="quick-cp" data-player="${index}" data-amount="1">+1</button>
            <button class="counter-btn plus" type="button" data-action="quick-cp" data-player="${index}" data-amount="2">+2</button>
          </div>
        </div>
      </div>
    </article>`;
...
function renderObjective(objective, index) {
  const game = state.game;
  const statusText = objective.status === 'p0' ? game.players[0].name : objective.status === 'p1' ? game.players[1].name : objective.status === 'contested' ? 'Contested' : 'Uncontrolled';
  return `
...
function renderScore() {
  ...
  const roundTotal = (round, player) => game.scoreEvents.filter(event => event.round === round && event.player === player).reduce((sum, event) => sum + Number(event.amount || 0), 0);
  const events = [...game.scoreEvents].reverse();

  return `
    <section class="score-summary">
      ${game.players.map((player, index) => `<article class="panel score-total"><strong>${esc(player.name)}</strong><span class="value">${totals[index]}</span><small>Victory Points</small></article>`).join('')}
    </section>
    ...
    <section class="panel gold">
      ...
      <form id="score-form" class="stack">
        <div class="grid-2">
          <div class="field"><label for="scorePlayer">Player</label><select id="scorePlayer" name="player">${game.players.map((player,index) => `<option value="${index}">${esc(player.name)}</option>`).join('')}</select></div>
        ...
        <div class="grid-2">
          <div class="field"><label for="scoreAmount">VP amount</label><input id="scoreAmount" name="amount" type="number" value="5" min="-100" max="100" required inputmode="numeric"></div>
    ...
    <section class="panel">
      ...
      <div style="overflow-x:auto">
        <table class="breakdown-table">
          <thead><tr><th>Round</th><th>${esc(game.players[0].name)}</th><th>${esc(game.players[1].name)}</th></tr></thead>
          <tbody>${rounds.map(round => `<tr><td>Round ${round}</td><td>${roundTotal(round,0)}</td><td>${roundTotal(round,1)}</td></tr>`).join('')}</tbody>
          <tfoot><tr><th>Total</th><th>${totals[0]}</th><th>${totals[1]}</th></tr></tfoot>
        </table>
      </div>
    </section>
    ...
    <section class="panel">
      ...
      ${events.length ? `<div class="event-list">${events.map(event => `
        <div class="event-item">
          <div class="event-main"><strong>${esc(game.players[event.player].name)} · ${esc(event.category)}</strong><span>Round ${event.round}, ${esc(event.phase)}${event.note ? ` · ${esc(event.note)}` : ''}</span></div>
  ...
  return `
    ...
    <section class="tools-grid">
      <article class="panel tool-card">
        ...
        <div class="grid-2">
          <div class="field"><label for="strength">Strength</label><input id="strength" type="number" min="1" max="99" value="4" inputmode="numeric"></div>
          <div class="field"><label for="toughness">Toughness</label><input id="toughness" type="number" min="1" max="99" value="4" inputmode="numeric"></div>
        </div>
  ...
  return `
    ...
    <section class="panel">
      ...
      <div class="info-list">
        ...
        <div class="info-row"><span>First turn each round</span><strong>${esc(game.players[game.firstPlayer].name)}</strong></div>
    ...
    <section class="panel blue">
      ...
      <label class="reminder-item">
        ...
        <div><strong>Automatic Command phase CP</strong><span>Add 1 CP to both players once at the start of each player’s Command phase.</span></div>
        <span></span>
      </label>
    </section>
    ...
    <section class="panel">
      ...
      ${cpEvents.length ? `<div class="event-list">${cpEvents.map(event => `
        <div class="event-item"><div class="event-main"><strong>${esc(game.players[event.player].name)} · ${esc(event.reason)}</strong><span>Round ${event.round}, ${esc(event.phase)}</span></div><span class="event-amount ${event.amount < 0 ? 'negative' : ''}">${event.amount > 0 ? '+' : ''}${event.amount}</span></div>`).join('')}</div>` : `<p class="help">No CP changes recorded.</p>`}
    </section>

    <section class="panel">
      <h2 class="section-title"><span>Completed Battles</span><small>${state.archives.length}</small></h2>
      ${state.archives.length ? `<div class="stack">${state.archives.map(item => `<div class="archive-item"><strong>${esc(item.players[0].name)} ${item.totals[0]} – ${item.totals[1]} ${esc(item.players[1].name)}</strong><span>${esc(item.mission)} · ${new Date(item.completedAt).toLocaleDateString()} · ${formatTime(item.duration)}</span></div>`).join('')}</div>` : `<p class="help">Completed battles will appear here.</p>`}
    </section>
    ...
    <div class="app-shell screen-complete" style="padding-bottom:0">
      <main class="complete-hero">
        <section class="complete-card">
          ...
          <div class="winner">${esc(winnerText(game))}</div>
          <div class="complete-score">
            <div><strong>${esc(game.players[0].name)}</strong><div class="score">${totals[0]}</div></div>
            <div class="versus">VS</div>
            <div><strong>${esc(game.players[1].name)}</strong><div class="score">${totals[1]}</div></div>
          </div>
...
function undoLast() {
  ...
  try {
    state.game = JSON.parse(snapshot);
    saveState();
    render();
...
app.addEventListener('submit', event => {
  ...
  if (event.target.id === 'score-form') {
    ...
    addScore(Number(data.get('player')), Number(data.get('amount')), String(data.get('category')), String(data.get('note') || '').trim());
  ...
  if (target.id === 'import-file' && target.files?.[0]) {
    ...
    reader.onload = () => {
      try {
        ...
        console.error(error);
        showToast('That file is not a valid WAR Companion backup.');
      }
    };
...
app.addEventListener('click', event => {
  ...
  if (action === 'quick-score') { addScore(Number(button.dataset.player), Number(button.dataset.amount), 'Quick'); return; }
  if (action === 'quick-cp') { adjustCP(Number(button.dataset.player), Number(button.dataset.amount)); return; }
  ...
  if (action === 'next-phase') {
    if (isFinalTurnPhase()) {
      if (confirm('End the battle and record the final score?')) finishGame();
    } else {
      advancePhase();
    }
    return;
  ...
  if (action === 'switch-turn') {
    if (confirm('Switch the active player without advancing the phase? Use this only to correct the tracker.')) {
      mutateGame(game => { game.activePlayer = 1 - game.activePlayer; grantCommandCP(game, true); });
    }
    return;
  }
  ...
  if (action === 'delete-score') {
    if (confirm('Delete this scoring entry?')) mutateGame(game => { game.scoreEvents = game.scoreEvents.filter(event => event.id !== button.dataset.id); });
    return;
  }
  ...
  if (action === 'finish-game') {
    if (confirm('Finish this battle and record the current score?')) finishGame();
    return;
  }
  if (action === 'export-data') {
    ...
    link.href = url;
    link.download = `war-companion-backup-${new Date().toISOString().slice(0,10)}.json`;
